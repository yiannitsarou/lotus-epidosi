# -*- coding: utf-8 -*-
"""
bhma7_v3.py — ΠΛΗΡΗΣ εκδοχή (με swaps) και συμβατή με το app.py μετά το Βήμα 6.

Κύρια σημεία
- Διατηρεί την λογική multi-class εξισορρόπησης ΕΠΙΔΟΣΗΣ (1/2/3) με “αντίθετα πρόσημα”.
- Φάση Α: Swaps ΔΥΑΔΩΝ (μόνο πλήρως αμοιβαίες από Βήμα 3, χωρίς κρίσιμες περιπτώσεις).
- Φάση Β: Swaps ΜΕΜΟΝΩΜΕΝΩΝ (χωρίς να σπάμε δυάδες, ίδιο category φύλο+γλώσσα).
- Veto κανόνες: όρια διαφοράς φύλου/γλώσσας (MAX_GENDER_DIFF, MAX_GREEK_DIFF),
  hard veto spread(επιδ_1)≤2 και spread(επιδ_3)≤2, soft veto για ίδια πρόσημα σε |Δ1|>1 & |Δ3|>1.
- Εξάγει:
  * Για κάθε σενάριο: νέα στήλη ΒΗΜΑ7_<ΣΕΝΑΡΙΟ> με ενημερωμένα τμήματα
  * Σύνοψη_ΒΗΜΑ7, Μετρήσεις_Επίδοσης, Ανταλλαγές

ΝΕΟ: Συνάρτηση apply_after_step6(step6_path[, out_path]) -> str
- Την καλεί το app.py αμέσως μετά το Βήμα 6.
- Εντοπίζει αυτόματα τα αρχεία Roster & Step3 στον ίδιο φάκελο με το step6 (ή χρησιμοποιεί
  default ονόματα αν υπάρχουν).
- Παραδίδει νέο Excel <step6_stem>_BHMA7v3.xlsx (ή στο out_path που δόθηκε).

ΣΗΜΕΙΩΣΗ: Δεν αλλάζουμε φύλα/γλώσσες, δεν σπάμε δυάδες, δεν παραβιάζουμε όρια.
"""
...
def _fmt_like_input(tag: str, greek_style: bool) -> str:
    ...
    return tag.replace("A","Α",1) if greek_style else tag
...
def build_allowed_dyads_from_step3(step3_xlsx: str, step3_sheet: Optional[str]=None) -> pd.DataFrame:
    """Δουλεύει πάνω στο αρχείο Βήματος 3 για να βρει ΠΛΗΡΩΣ αμοιβαίες δυάδες που
       δεν είναι «κρίσιμες» (παιδί εκπ/κού, ιδιαιτερότητα, ζωηρός Ν1/Ν2/Ν3).
    """
...
def _violates_balance_after_swap(df: pd.DataFrame, class_col: str,
    ...
    for cls_a, cls_b in itertools.combinations(all_classes, 2):
        ...
        if abs(d1) > 1 and abs(d3) > 1 and sgn(d1) == sgn(d3):
            return True
    return False
...
def run_step7_for_scenario(step6_xlsx: str, roster_xlsx: str, step3_xlsx: str,
                           ...
                           target_diff: int=TARGET_DIFF, max_swaps: int=MAX_SWAPS):
    """Εφαρμογή λογικής Βήματος 7 σε ένα φύλλο/σενάριο του Βήματος 6."""
    ...
    def _pair_perf_counts(a, b):
        ...
        return {"c1": int((pa==1) + (pb==1)), "c3": int((pa==3) + (pb==3))}
    ...
    def pick_classes_to_swap(c1: Dict[str,int], c3: Dict[str,int]):
        ...
        donor_by_1 = items_1[-1][0]; receiver_by_1 = items_1[0][0]
        donor_by_3 = items_3[-1][0]; receiver_by_3 = items_3[0][0]
        ...
        return (donor_by_1, receiver_by_1) if spread_opt1 >= spread_opt2 else (donor_by_3, receiver_by_3)
    ...
    if ENABLE_DYAD_SWAPS:
        for _ in range(max_swaps):
            ...
            if spread_1 <= target_diff and spread_3 <= target_diff: break
            ...
            for cat in cats:
                ...
                for _, li in L.iterrows():
                    for _, rj in R.iterrows():
                        ...
                        li_c = li["perf_counts"]; rj_c = rj["perf_counts"]
                        ...
                        trial_1 = cur_counts_1.copy(); trial_1[donor] = int(post_donor_1); trial_1[receiver] = int(post_recv_1)
                        trial_3 = cur_counts_3.copy(); trial_3[donor] = int(post_donor_3); trial_3[receiver] = int(post_recv_3)
    ...
    if ENABLE_SINGLE_SWAPS and swaps_done < max_swaps:
        ...
        while swaps_done < max_swaps:
            ...
            if not Sd_1.empty and not Sr_3.empty:
                for _, a in Sd_1.iterrows():
                    for _, b in Sr_3.iterrows():
                        if not _same_category(a["ΦΥΛΟ"], a["ΚΑΛΗ_ΓΝΩΣΗ_ΕΛΛΗΝΙΚΩΝ"], b["ΦΥΛΟ"], b["ΚΑΛΗ_ΓΝΩΣΗ_ΕΛΛΗΝΙΚΩΝ"]):
                            continue
                        ...
                        trial_1 = cur_counts_1.copy(); trial_1[donor] = post_d1; trial_1[receiver] = post_r1
                        trial_3 = cur_counts_3.copy(); trial_3[donor] = post_d3; trial_3[receiver] = post_r3
            ...
            # Εφαρμογή
            mask_a = names==best["a_name"]
            mask_b = names==best["b_name"]
            b7.loc[mask_a] = _class_style_tag(receiver, greek_style)
            b7.loc[mask_b] = _class_style_tag(donor, greek_style)
            ...
            work2.loc[mask_a, "CLASS"] = receiver
            work2.loc[mask_b, "CLASS"] = donor
    ...
    cur_counts_2 = _counts_by_level(b7.map(lambda x: x.replace("Α","A",1) if x else None), pd.Series(perf), 2)
...
def _auto_locate_inputs(step6_path: Path) -> tuple[str, str]:
    """Προσπαθεί να εντοπίσει Roster και Step3 στον ίδιο φάκελο με το step6."""
    folder = step6_path.parent
    # Υποψήφια για Roster
    roster_candidates = [
        "Παραδειγμα1.xlsx", "Roster.xlsx", "input.xlsx", "roster.xlsx"
    ] + [p.name for p in folder.glob("*Roster*.xlsx")] + [p.name for p in folder.glob("*Παραδειγμ*.xlsx")]
    # Υποψήφια για Step3
    step3_candidates = [
        "STEP3_SCENARIOS.xlsx", "step3_amivaia_filia_FIXED.xlsx"
    ] + [p.name for p in folder.glob("*STEP3*.xlsx")] + [p.name for p in folder.glob("*amivia*3*.xlsx")]

    roster = next((str(folder / c) for c in roster_candidates if (folder / c).exists()), None)
    step3  = next((str(folder / c) for c in step3_candidates if (folder / c).exists()), None)
    if not roster:
        raise FileNotFoundError("Δεν βρέθηκε αρχείο Roster δίπλα στο Step6. Ονόμασέ το π.χ. 'Παραδειγμα1.xlsx' ή 'Roster.xlsx'.")
    if not step3:
        raise FileNotFoundError("Δεν βρέθηκε αρχείο Βήματος 3 δίπλα στο Step6. Ονόμασέ το π.χ. 'STEP3_SCENARIOS.xlsx'.")
    return roster, step3


def apply_after_step6(step6_path: str, out_path: Optional[str] = None,
                      roster_xlsx: Optional[str] = None, step3_xlsx: Optional[str] = None,
                      target_diff: int = TARGET_DIFF, max_swaps: int = MAX_SWAPS) -> str:
    """
    Καλείται από το app.py αμέσως μετά το Βήμα 6.
    - step6_path: Excel του Βήματος 6 (πολλαπλά φύλλα/σενάρια)
    - out_path:   (προαιρετικό) τελικό Excel που θα παραχθεί
    - roster_xlsx/step3_xlsx: (προαιρετικά) explicit paths αν δεν εντοπιστούν αυτόματα
    - target_diff/max_swaps: ρυθμίσεις αλγορίθμου
    Επιστρέφει: path του νέου Excel.
    """
    step6 = Path(step6_path).resolve()
    if not step6.exists():
        raise FileNotFoundError(f"Το αρχείο Βήματος 6 δεν βρέθηκε: {step6}")
    if roster_xlsx is None or step3_xlsx is None:
        r, s3 = _auto_locate_inputs(step6)
        roster_xlsx = roster_xlsx or r
        step3_xlsx  = step3_xlsx  or s3

    out = Path(out_path).resolve() if out_path else step6.with_name(step6.stem + "_BHMA7v3.xlsx")

    s6 = pd.ExcelFile(step6)
    sheets = [s for s in s6.sheet_names if isinstance(s, str) and s.strip()]
    skip_tokens = {"ΣΥΝΟΨΗ", "SUMMARY", "ΜΕΤΡΗΣΕΙΣ", "COUNTS"}
    scenarios = [s for s in sheets if not any(tok in s.upper() for tok in skip_tokens)]

    all_summ, all_counts, all_swaps = [], [], []

    with pd.ExcelWriter(out, engine="xlsxwriter") as writer:
        for scen in scenarios:
            try:
                out_df, summary, counts_df, swaps_log = run_step7_for_scenario(
                    str(step6), str(roster_xlsx), str(step3_xlsx), scen, None, target_diff, max_swaps
                )
                ...
                pd.DataFrame([{"Σενάριο": scen, "Σφάλμα": str(e)}]).to_excel(
                    writer, index=False, sheet_name=scen
                )
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Χρήση: python bhma7_v3.py <STEP6.xlsx> [OUTPUT.xlsx] [ROSTER.xlsx] [STEP3.xlsx]")
        sys.exit(1)
    inp = sys.argv[1]
    outp = sys.argv[2] if len(sys.argv) > 2 else None
    roster = sys.argv[3] if len(sys.argv) > 3 else None
    step3  = sys.argv[4] if len(sys.argv) > 4 else None
    res = apply_after_step6(inp, outp, roster, step3)
    print("Παράχθηκε:", res)
